...
void	check_if_surrounded_by_walls(t_main_struct *boss, int y_max_len)
{
	...
	{
		ft_printf("i -> %i\n(ft_strlen(boss->win.mapa[%i]) - 1) -> %i\n",
			i, i, (ft_strlen(boss->win.mapa[i]) - 2));
		ft_printf("The character -> %c;\n", boss->win.mapa[i][(ft_strlen(boss->win.mapa[i]) - 2)]);
		if (boss->win.mapa[i][0] == ' ')
			error_call("The Map is not a rectangle", boss);
		if (boss->win.mapa[i][0] != '1')
			error_call("Map not surrounded by walls", boss);
		if (boss->win.mapa[i][(ft_strlen(boss->win.mapa[i]))] == ' ')
			error_call("The Map is not a rectangle", boss);
		if (boss->win.mapa[i][(ft_strlen(boss->win.mapa[i]) - 2)] != '1')
			error_call("Map not surrounded by walls trig", boss);
	}
